...
public class Solution {
    ...
    public static class Politician extends Thread {
        private volatile int countSounds; //Ключевое слово volatile - это еще один способ (как synchronized, atomic wrapper) сделать класс потокобезопасным (Thread safe).
                                            //volatile объясняет компилятору что значение переменной никогда не должно кэшироваться, потому что это значение может быть изменено снаружи
